...
#Steps
  DOCLEAN = 1 #Delete created files(before and after running tasks).DOCONVERT = 1 #Convert AliESDs.root to AO2D.root.DOALI = 1 #Run AliPhysics tasks.DOO2 = 1 #Run O2 tasks.DOPOSTPROCESS = 1 #Run output postprocessing.(Comparison plots.Requires DOALI = 1 and / or DOO2 = 1)
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      DOO2_FDDCONV = 0 #fdd - converter
                                                                                                                                                                                                                                                DOO2_COLLCONV = 0 #collision - converter
                                                                                                                                                                                                                                                                                 DOO2_ZDC = 1 #zdc - converter
                                                                                                                                                                                                                                                                                                       DOO2_TRACKSELE = 1 #trackselection,
                                                             dca, propagation
...
#O2 database
DATABASE_O2 = "workflows.yml" MAKE_GRAPH = 0 #Make topology graph.
...
#User tasks
  ...
  SAVETREES = 0 #Save O2 tables to trees.USEO2VERTEXER = 0 #Use the O2 vertexer in AliPhysics.USEALIEVCUTS = 1 #Use AliEventCuts in
  AliPhysics(as used by conversion task)
...
#Cleanup before running
  ["$1" - eq 1]&&
  {
    bash "$DIR_TASKS/clean.sh" || ErrExit;
  }
...
#Cleanup after running
  ["$1" - eq 2]&&
  {
    rm - f
        ...
        "$SCRIPT_O2"
        "$SCRIPT_POSTPROCESS" ||
      ErrExit "Failed to rm created files."
        ["$JSON_EDIT"]&&
    {
      rm "$JSON_EDIT" || ErrExit "Failed to rm $JSON_EDIT.";
    }
...
#Make a copy of the default JSON file to modify it.
  JSON_EDIT = "${JSON/.json/_edit.json}" cp
              "$JSON"
              "$JSON_EDIT" ||
              ErrExit "Failed to cp $JSON $JSON_EDIT." JSON = "$JSON_EDIT"

#Collision system
    MsgWarn "Setting collision system $INPUT_SYS"

#Run 2 / 3 / 5
    MsgWarn "Using Run $INPUT_RUN" if["$INPUT_RUN" - eq 2];
  then
      ReplaceString
    "\"processRun2\": \"false\""
    "\"processRun2\": \"true\""
    "$JSON" ||
    ErrExit "Failed to edit $JSON." ReplaceString
            "\"processRun3\": \"true\""
            "\"processRun3\": \"false\""
            "$JSON" ||
    ErrExit "Failed to edit $JSON." elif["$INPUT_RUN" - eq 3];
  then
      ReplaceString
    "\"processRun2\": \"true\""
    "\"processRun2\": \"false\""
    ...
    ErrExit "Failed to edit $JSON." ReplaceString
            ...
            "\"processRun3\": \"true\""
            "$JSON" ||
    ErrExit "Failed to edit $JSON." fi

#MC
    if["$ISMC" - eq 1];
  then
      MsgWarn "Using MC data" ReplaceString
              "\"processMc\": \"false\""
              "\"processMc\": \"true\""
              "$JSON" ||
    ErrExit "Failed to edit $JSON." ReplaceString
    ...
    ErrExit "Failed to edit $JSON." ReplaceString
            ...
            "\"isMC\": \"true\""
            "$JSON" ||
    ErrExit "Failed to edit $JSON." else MsgWarn "Using real data" ReplaceString
                                                 "\"processMc\": \"true\""
                                                 "\"processMc\": \"false\""
                                                 "$JSON" ||
    ErrExit "Failed to edit $JSON." ReplaceString
...
#event - selection
      ReplaceString
            ...
            "\"syst\": \"$INPUT_SYS\""
            "$JSON" ||
    ErrExit "Failed to edit $JSON."

    if[$DOO2_TRIGSEL - eq 1];
  then
#trigger selection
      ReplaceString
    ...
    ErrExit "Failed to edit $JSON." ReplaceString
            ...
            "\"processNoTrigSel\": \"false\""
            "$JSON" ||
    ErrExit "Failed to edit $JSON." fi if["$INPUT_RUN" - eq 3];
  then
...
#timestamp - task
    if[["$ISMC" - eq 1 && "$INPUT_RUN" - eq 2]];
  then
      ReplaceString
    "\"isRun2MC\": \"false\""
    "\"isRun2MC\": \"true\""
    "$JSON" ||
    ErrExit "Failed to edit $JSON." else ReplaceString
    "\"isRun2MC\": \"true\""
    "\"isRun2MC\": \"false\""
    "$JSON" ||
    ErrExit "Failed to edit $JSON." fi
...
#Make a copy of the default workflow database file before modifying it.
    DATABASE_O2_EDIT = "${DATABASE_O2/.yml/_edit.yml}" cp
                       "$DATABASE_O2"
                       "$DATABASE_O2_EDIT" ||
                       ErrExit "Failed to cp $DATABASE_O2 $DATABASE_O2_EDIT." DATABASE_O2 = "$DATABASE_O2_EDIT"
...
#Replace the workflow version masks with the actual values in the workflow database.
                                                                                            ...
                                                                                            "$SUFFIX_RUN"
                                                                                            "$DATABASE_O2" ||
                                                                                            ErrExit "Failed to edit $DATABASE_O2."

#Generate the O2 command.
                                                                                            MAKECMD = "python3 $DIR_EXEC/make_command_o2.py $DATABASE_O2 $OPT_MAKECMD" O2EXEC = $($MAKECMD - w "$WORKFLOWS")
                                                                                                                                                                                      $MAKECMD -
                                                                                                                                                                                    w "$WORKFLOWS" 1 >
                                                                                                                                                                                  / dev / null 2 > / dev / null ||
                                                                                                                                                                                ErrExit "Generating of O2 command failed."
                                                                                                                                                                                  ["$O2EXEC"] ||
                                                                                                                                                                                ErrExit "Nothing to do!"
...
#Create the script with the full O2 command.
                                                                                                                                                                                  ...
                                                                                                                                                                                  "$SCRIPT_O2"
#!/ bin / bash
                                                                                                                                                                                  FileIn = "\$1" JSON = "\$2" $O2EXEC
                                                                                                                                                                                  EOF
}

function MakeScriptAli{
  ALIEXEC = "root -b -q -l \"$DIR_TASKS/RunJetTaskLocal.C(\\\"\$FileIn\\\", \\\"\$JSON\\\", $ISMC, $USEO2VERTEXER, $USEALIEVCUTS)\"" cat << EOF > "$SCRIPT_ALI"
#!/ bin / bash
            FileIn = "\$1" JSON = "\$2" $ALIEXEC
              EOF}
