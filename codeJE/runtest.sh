...
#Load utilities.
source "$DIR_EXEC/utilities.sh" ||
{
  echo "Error: Failed to load utilities.";
  exit 1;
}
...
function Help
{
  echo "Usage: bash [<path>/]$(basename " $0 ") [-h] [-i <input config>] [-t <task config>] [-d]";
}
while
  getopts ":hi:t:d" opt;
do
  case ${opt} in
    ...
    :)
      MsgErr "Invalid option: $OPTARG requires an argument." 1>&2;
  Help;
  exit 1;
  ;
  esac
...
#Load input specification.
        source "$CONFIG_INPUT" ||
    ErrExit "Failed to load input specification."

#Load tasks configuration.
    source "$CONFIG_TASKS" ||
    ErrExit "Failed to load tasks configuration." DIR_TASKS = "$(dirname " $(realpath "$CONFIG_TASKS") ")"
  ...
  {
    echo "$0";
    echo "Input specification: $CONFIG_INPUT";
    echo "Tasks configuration: $CONFIG_TASKS";
  }
...
#Clean before running.
    if[$DOCLEAN - eq 1];
  then
      MsgStep "Cleaning..." Clean 1 ||
    ErrExit "Clean failed." fi

#Generate list of input files.
      MsgStep "Generating list of input files..."
        [$INPUT_IS_O2 - eq 1]&& LISTFILES = "$LISTFILES_O2" || LISTFILES = "$LISTFILES_ALI" INPUT_DIR = "$(realpath " $INPUT_DIR ")"
                                              [$DEBUG - eq 1]&&
  {
    echo "Searching for $INPUT_FILES in $INPUT_DIR";
  }
  find "$INPUT_DIR" - name "$INPUT_FILES" | sort | head - n $NFILESMAX > "$LISTFILES" [[${PIPESTATUS[0]} - eq 0 || ${PIPESTATUS[0]} - eq 141]] || ErrExit "Failed to make a list of input files."
                                                                                                                                                    ["$(wc -l < " $LISTFILES ")" - eq 0]&& { ErrExit "No input files!"; }

#Modify the JSON file.
  MsgStep "Modifying JSON file..." CheckFile "$JSON" AdjustJson || ErrExit "AdjustJson failed." CheckFile "$JSON"

#Convert AliESDs.root to AO2D.root.
    if[$DOCONVERT - eq 1];
  then
    CheckFile "$LISTFILES_ALI" NFILES = $(wc - l < "$LISTFILES_ALI")
  ...
  NFILESPERJOB_CONVERT = $(python3 - c "n = $NFILESPERJOB_CONVERT; print(n if n > 0 else max(1, round($NFILES * $NCORESPERJOB_ALI / $NCORES)))")
                           MsgStep "Converting... ($NFILES files)"
                             [$INPUT_IS_MC - eq 1] &&
                         MsgWarn "Using MC mode"
                           [$DEBUG - eq 1] &&
                         echo "Loading AliPhysics..."
#Run the batch script in the ALI environment.
                           ["$O2_ROOT"]&&
  {
    MsgWarn "O2 environment is loaded - expect errors!";
  }
  ["$ALICE_PHYSICS"]&&
  {
    MsgWarn "AliPhysics environment is already loaded.";
    ENV_ALI = "";
  }
  $ENV_ALI bash
    "$DIR_EXEC/batch_convert.sh"
    "$LISTFILES_ALI"
    "$LISTFILES_O2" $INPUT_IS_MC $USEALIEVCUTS $DEBUG "$NFILESPERJOB_CONVERT" ||
    exit 1 fi
...
#Run AliPhysics tasks.
  ...
  NFILESPERJOB_ALI = $(python3 - c "n = $NFILESPERJOB_ALI; print(n if n > 0 else max(1, round($NFILES * $NCORESPERJOB_ALI / $NCORES)))")
                         MsgStep "Running AliPhysics tasks... ($NFILES files)" rm -
                       f
                       "$FILEOUT"
                       "$FILEOUT_ALI" ||
                     ErrExit "Failed to rm $FILEOUT $FILEOUT_ALI." MakeScriptAli || ErrExit "MakeScriptAli failed." CheckFile "$SCRIPT_ALI"[$DEBUG - eq 1] && echo "Loading AliPhysics..."
#Run the batch script in the ALI environment.
                                                                                                                                                                ["$O2_ROOT"]&&
  {
    MsgWarn "O2 environment is loaded - expect errors!";
  ...
  $ENV_ALI bash
    "$DIR_EXEC/batch_ali.sh"
    "$LISTFILES_ALI"
    "$JSON"
    "$SCRIPT_ALI" $DEBUG "$NFILESPERJOB_ALI" ||
    exit 1 mv
    "$FILEOUT"
    "$FILEOUT_ALI" ||
    ErrExit "Failed to mv $FILEOUT $FILEOUT_ALI." fi

#Run O2 tasks.
    if[$DOO2 - eq 1];
  then
    CheckFile "$LISTFILES_O2" NFILES = $(wc - l < "$LISTFILES_O2")
      ["$NFILES" - eq 0]&& { ErrExit "No input O2 files!"; }
  NFILESPERJOB_O2 = $(python3 - c "n = $NFILESPERJOB_O2; print(n if n > 0 else min(16, max(1, round($NFILES * $NCORESPERJOB_O2 / $NCORES))))") #FIXME : Jobs with more than 16 files per job lose data.MsgStep "Running O2 tasks... ($NFILES files)" rm - f
                      "$FILEOUT"
                      "$FILEOUT_O2" ||
                    ErrExit "Failed to rm $FILEOUT $FILEOUT_O2." MakeScriptO2 || ErrExit "MakeScriptO2 failed." CheckFile "$SCRIPT_O2"[$SAVETREES - eq 1] || FILEOUT_TREES = ""
                                                                                                                                                                               [$DEBUG - eq 1] &&
                                                                                                                                                                             echo "Loading O2Physics..."
#Run the batch script in the O2 environment.
                                                                                                                                                                               ["$ALICE_PHYSICS"]&&
  {
    MsgWarn "AliPhysics environment is loaded - expect errors!";
  }
  ["$O2_ROOT"]&&
  {
    MsgWarn "O2 environment is already loaded.";
    ENV_O2 = "";
  }
  $ENV_O2 bash
    "$DIR_EXEC/batch_o2.sh"
    "$LISTFILES_O2"
    "$JSON"
    "$SCRIPT_O2" $DEBUG
    "$NFILESPERJOB_O2"
    ...
    "$FILEOUT"
    "$FILEOUT_O2" ||
    ErrExit "Failed to mv $FILEOUT $FILEOUT_O2."
    [[$SAVETREES - eq 1 && "$FILEOUT_TREES"]]&&
  {
    mv "$FILEOUT_TREES"
       "$FILEOUT_TREES_O2" ||
      ErrExit "Failed to mv $FILEOUT_TREES $FILEOUT_TREES_O2.";
  }
  fi

#Run output postprocessing.(Compare AliPhysics and O2 output.)
    if[$DOPOSTPROCESS - eq 1];
  then
    LogFile = "log_postprocess.log" MsgStep "Postprocessing... (logfile: $LogFile)" MakeScriptPostprocess || ErrExit "MakeScriptPostprocess failed." CheckFile "$SCRIPT_POSTPROCESS"
                                                                                                                 [$DEBUG - eq 1] &&
                                                                                                               echo "Loading ROOT..."
#Run the batch script in the postprocessing environment.
                                                                                                                 ["$ROOTSYS"]&&
  {
    MsgWarn "ROOT environment is already loaded.";
    ENV_POST = "";
  }
  $ENV_POST bash
      "$SCRIPT_POSTPROCESS"
      "$FILEOUT_O2"
      "$FILEOUT_ALI" > $LogFile 2 > &1 ||
    ErrExit "\nCheck $(realpath $LogFile)" grep - q - e '^' "W-" - e '^' "Warning" - e
        "warning"
        "$LogFile" &&
      MsgWarn "There were warnings!\nCheck $(realpath $LogFile)" grep - q - e '^' "E-" - e '^'
        "Error"
        "$LogFile" &&
      MsgErr "There were errors!\nCheck $(realpath $LogFile)" grep - q - e '^' "F-" - e '^' "Fatal" - e "segmentation" - e
        "Segmentation"
        "$LogFile" &&
      ErrExit "There were fatal errors!\nCheck $(realpath $LogFile)" fi

#Clean after running.
      if[$DOCLEAN - eq 1];
  then
      MsgStep "Cleaning..." Clean 2 ||
    ErrExit "Clean failed." fi
