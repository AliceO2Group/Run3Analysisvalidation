...
Int_t PlotEfficiencyRecoStep(TString pathFile = "AnalysisResults_O2.root", TString particles = "d0")
{
  ...
  for (int iP = 0; iP < arrayParticle->GetEntriesFast(); iP++) {
    particle = (reinterpret_cast<TObjString*>(arrayParticle->At(iP)))->GetString();
    if (!particle.Length()) {
    ...
    TH2F* hPtVsYGenIncl = reinterpret_cast<TH2F*>(file->Get(nameHistgen.Data()));
    if (!hPtVsYGenIncl) {
    ...
    TH1F* hGenPtIncl = reinterpret_cast<TH1F*>(hPtVsYGenIncl->ProjectionX("hGen_Pt", 1, hPtVsYGenIncl->GetXaxis()->GetLast(), "e"));
    TH1F* hGenYIncl = reinterpret_cast<TH1F*>(hPtVsYGenIncl->ProjectionY("hGen_Y", 1, hPtVsYGenIncl->GetYaxis()->GetLast(), "e"));

    if (iNRebin > 1) {
      hGenPtIncl->Rebin(iNRebin);
      hGenYIncl->Rebin(iNRebin);
    } else if (NRebin > 1) {
      hGenPtIncl = reinterpret_cast<TH1F*>(hGenPtIncl->Rebin(NRebin, "hGenPtInclR", dRebin));
      hGenYIncl = reinterpret_cast<TH1F*>(hGenYIncl->Rebin(NRebin, "hGenYInclR", dRebin));
    }
    ...
    TH2F* hPtVsYGenPrompt = reinterpret_cast<TH2F*>(file->Get(nameHistgenPrompt.Data()));
    if (!hPtVsYGenPrompt) {
    ...
    TH1F* hGenPtPrompt = reinterpret_cast<TH1F*>(hPtVsYGenPrompt->ProjectionX("hGenPrompt_Pt", 1, hPtVsYGenPrompt->GetXaxis()->GetLast(), "e"));
    TH1F* hGenYPrompt = reinterpret_cast<TH1F*>(hPtVsYGenPrompt->ProjectionY("hGenPrompt_Y", 1, hPtVsYGenPrompt->GetYaxis()->GetLast(), "e"));

    if (iNRebin > 1) {
      hGenPtPrompt->Rebin(iNRebin);
      hGenYPrompt->Rebin(iNRebin);
    } else if (NRebin > 1) {
      hGenPtPrompt = reinterpret_cast<TH1F*>(hGenPtPrompt->Rebin(NRebin, "hGenPtPromptR", dRebin));
      hGenYPrompt = reinterpret_cast<TH1F*>(hGenYPrompt->Rebin(NRebin, "hGenYPromptR", dRebin));
    }
    ...
    TH2F* hPtVsYGenNonPrompt = reinterpret_cast<TH2F*>(file->Get(nameHistgenNonPrompt.Data()));
    ...
    TH1F* hGenPtNonPrompt = reinterpret_cast<TH1F*>(hPtVsYGenNonPrompt->ProjectionX("hGenNonPrompt_Pt", 1, hPtVsYGenNonPrompt->GetXaxis()->GetLast(), "e"));
    TH1F* hGenYNonPrompt = reinterpret_cast<TH1F*>(hPtVsYGenNonPrompt->ProjectionY("hGenNonPrompt_Y", 1, hPtVsYGenNonPrompt->GetYaxis()->GetLast(), "e"));

    if (iNRebin > 1) {
      hGenPtNonPrompt->Rebin(iNRebin);
      hGenYNonPrompt->Rebin(iNRebin);
    } else if (NRebin > 1) {
      hGenPtNonPrompt = reinterpret_cast<TH1F*>(hGenPtNonPrompt->Rebin(NRebin, "hGenPtNonPromptR", dRebin));
      hGenYNonPrompt = reinterpret_cast<TH1F*>(hGenYNonPrompt->Rebin(NRebin, "hGenYNonPromptR", dRebin));
    }
    ...
    for (int iRs = 0; iRs < NRecoStep; iRs++) {
      ...
      TH2F* hPtVsYRecIncl = reinterpret_cast<TH2F*>(file->Get(nameHistRec.Data()));
      if (!hPtVsYRecIncl) {
      ...
      TH1F* hRecoPtIncl = reinterpret_cast<TH1F*>(hPtVsYRecIncl->ProjectionX(Form("h%s_Pt", recoStep[iRs].Data()), 1, hPtVsYRecIncl->GetXaxis()->GetLast(), "e"));
      canRecoSbySPt->cd(iRs + 1);
      hRecoPtIncl->Draw("pe");

      TH1F* hRecoYIncl = reinterpret_cast<TH1F*>(hPtVsYRecIncl->ProjectionY(Form("h%s_Y", recoStep[iRs].Data()), 1, hPtVsYRecIncl->GetYaxis()->GetLast(), "e"));
      ...
      if (iNRebin > 1) {
        ...
        hRecoPtIncl = reinterpret_cast<TH1F*>(hRecoPtIncl->Rebin(NRebin, "hRecoPtInclR", dRebin));
        hRecoYIncl = reinterpret_cast<TH1F*>(hRecoYIncl->Rebin(NRebin, "hRecoYInclR", dRebin));
      }

      hEffPtIncl[iRs] = reinterpret_cast<TH1F*>(hRecoPtIncl->Clone(Form("hEffPtIncl%s", recoStep[iRs].Data())));
      ...
      hEffYIncl[iRs] = reinterpret_cast<TH1F*>(hRecoYIncl->Clone(Form("hEffYIncl%s", recoStep[iRs].Data())));
      ...
      TH2F* hPtVsYRecPrompt = reinterpret_cast<TH2F*>(file->Get(nameHistRecPrompt.Data()));
      if (!hPtVsYRecPrompt) {
      ...
      TH1F* hRecoPtPrompt = reinterpret_cast<TH1F*>(hPtVsYRecPrompt->ProjectionX(Form("h%s_PtPrompt", recoStep[iRs].Data()), 1, hPtVsYRecPrompt->GetXaxis()->GetLast(), "e"));
      canRecoSbySPromptPt->cd(iRs + 1);
      hRecoPtPrompt->Draw("pe");

      TH1F* hRecoYPrompt = reinterpret_cast<TH1F*>(hPtVsYRecPrompt->ProjectionY(Form("h%s_YPrompt", recoStep[iRs].Data()), 1, hPtVsYRecPrompt->GetYaxis()->GetLast(), "e"));
      ...
      if (iNRebin > 1) {
        ...
        hRecoPtPrompt = reinterpret_cast<TH1F*>(hRecoPtPrompt->Rebin(NRebin, "hRecoPtPromptR", dRebin));
        hRecoYPrompt = reinterpret_cast<TH1F*>(hRecoYPrompt->Rebin(NRebin, "hRecoYPromptR", dRebin));
      }

      hEffPtPrompt[iRs] = reinterpret_cast<TH1F*>(hRecoPtPrompt->Clone(Form("hEffPtPrompt%s", recoStep[iRs].Data())));
      ...
      hEffYPrompt[iRs] = reinterpret_cast<TH1F*>(hRecoYPrompt->Clone(Form("hEffYPrompt%s", recoStep[iRs].Data())));
      ...
      TH2F* hPtVsYRecNonPrompt = reinterpret_cast<TH2F*>(file->Get(nameHistRecNonPrompt.Data()));
      ...
      TH1F* hRecoPtNonPrompt = reinterpret_cast<TH1F*>(hPtVsYRecNonPrompt->ProjectionX(Form("h%s_PtNonPrompt", recoStep[iRs].Data()), 1, hPtVsYRecNonPrompt->GetXaxis()->GetLast(), "e"));
      canRecoSbySNonPromptPt->cd(iRs + 1);
      hRecoPtNonPrompt->Draw("pe");

      TH1F* hRecoYNonPrompt = reinterpret_cast<TH1F*>(hPtVsYRecNonPrompt->ProjectionY(Form("h%s_YNonPrompt", recoStep[iRs].Data()), 1, hPtVsYRecNonPrompt->GetYaxis()->GetLast(), "e"));
      ...
      if (iNRebin > 1) {
        ...
        hRecoPtNonPrompt = reinterpret_cast<TH1F*>(hRecoPtNonPrompt->Rebin(NRebin, "hRecoPtNonPromptR", dRebin));
        hRecoYNonPrompt = reinterpret_cast<TH1F*>(hRecoYNonPrompt->Rebin(NRebin, "hRecoYNonPromptR", dRebin));
      }

      hEffPtNonPrompt[iRs] = reinterpret_cast<TH1F*>(hRecoPtNonPrompt->Clone(Form("hEffPtNonPrompt%s", recoStep[iRs].Data())));
      ...
      hEffYNonPrompt[iRs] = reinterpret_cast<TH1F*>(hRecoYNonPrompt->Clone(Form("hEffYNonPrompt%s", recoStep[iRs].Data())));
