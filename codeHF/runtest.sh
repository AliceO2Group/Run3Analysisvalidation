...
ls $INPUTDIR/$STRING > $LISTFILESALI || { MsgErr "Error: Failed to make a list of input files."; exit 1; }
...
if [ $DOCONVERT -eq 1 ]; then
  [ -f "$LISTFILESALI" ] || { MsgErr "\nConverting: Error: File $LISTFILESALI does not exist."; exit 1; }
  MsgStep "Converting... ($(cat $LISTFILESALI | wc -l) files)"
  ...
  if [ $CONVSEP -eq 1 ]; then
    ...
    $ENVALI $CMDROOT "convertAO2D.C(\"$LISTFILESALI\", $ISMC, $NMAX)" > $LOGFILE 2>&1 || { MsgErr "Error"; exit 1; }
...
$O2EXEC
EOF
  #$ENVO2 bash $O2SCRIPT > $LOGFILE 2>&1 || { MsgErr "Error"; exit 1; } # Run the script in the O2 environment.
...
if [ $DORUN1 -eq 1 ]; then
  ...
  MsgStep "Running the HF tasks with AliPhysics... ($(cat $LISTFILESALI | wc -l) files)"
...
if [ $DOCOMPARE -eq 1 ]; then
  ...
  [ $ok -ne 1 ] && exit 1
  $ENVALI $CMDROOT "Compare.C(\"$FILEOUTO2\",\"$FILEOUTALI\", $MASS)" > $LOGFILE 2>&1 || { MsgErr "Error"; exit 1; }
fi
