...
def LoadInputs(inputCfg):
  ...
  Parameters
  ----------
  - inputCfg: dictionary from yaml config file 

  Returns
  ----------
  - histos: dictionary with input histos, keys (rawyidls, acceffp, acceffnp)
...
def Main():
  ...
  gFraction.SetNameTitle('gFraction', ';#it{p}_{T} (GeV/#it{c});#if{f}_{prompt}')
  hPtSpectrum = TH1F('hPtSpectrum', ';#it{p}_{T} (GeV/#it{c});d#sigma/d#it{p}_{T} (pb GeV^{-1} #it{c})',
                     len(ptLims['rawyields'])-1, ptLims['rawyields'])
  hPtSpectrumWoBR = TH1F('hPtSpectrumWoBR', ';#it{p}_{T} (GeV/#it{c});d#sigma/d#it{p}_{T} #times BR (pb GeV^{-1} #it{c})',
                         len(ptLims['rawyields'])-1, ptLims['rawyields'])
  ...
  outFile = TFile(cfg['output']['filename'], 'recreate')
  hPtSpectrum.Write()
