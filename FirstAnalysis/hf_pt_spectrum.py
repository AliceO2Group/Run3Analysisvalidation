...
def load_inputs(input_cfg):
    ...
    if not histos['acceffp']:
        print(f'\033[91mERROR: prompt (acc x eff) histo {effprompt_hist_name}'
    ...
    if not histos['acceffnp']:
        print(f'\033[91mERROR: nonprompt (acc x eff) histo {effprompt_hist_name}'
              f'not found in {eff_file_name}. Exit\033[0m')
        sys.exit()
    ...
    norm['sigmaMB'] = norm_db['sigma']['Run2'][system][energy] if observable == 'dsigmadpt' else 1.
...
def main():
    ...
    parser.add_argument('configfile_name', metavar='text',
    ...
    hptspectrum = TH1F('hptspectrum', f';{axistit_pt};{axistit_cross}',
                       len(ptlims['rawyields'])-1, ptlims['rawyields'])
    hptspectrum_wo_br = TH1F('hptspectrum_wo_br', f';{axistit_pt};{axistit_cross_times_br}',
                             len(ptlims['rawyields'])-1, ptlims['rawyields'])
    ...
    for i_pt, (ptmin, ptmax) in enumerate(zip(ptlims['rawyields'][:-1], ptlims['rawyields'][1:])):
        ...
        crosssec_nonprompt_fonll = [histos['FONLL']['nonprompt'][pred].Integral(
        ...
        if frac_method == 'Nb':
            ...
            crosssec_prompt_fonll = [histos['FONLL']['prompt'][pred].Integral(
                ptmin_fonll, ptmax_fonll, 'width'
