  ...
  bool savePlots = true)
{
  ...
  for (int iMult = 1; iMult < nBinsMult; iMult++) {
    ...
    if (savePlots) canDiffFlow_v2->SaveAs(Form("%s/flow/v2_mult%d.png", outputPlotsName, iMult));
  ...
  if (savePlots) canRefFlow_v2->SaveAs(Form("%s/flow/v2.png", outputPlotsName));
