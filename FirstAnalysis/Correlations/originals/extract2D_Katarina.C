...
void extract2D(const char* fileName = "AnalysisResults_O2.root", const char* outputFile = "dphi_corr.root", const char* folder = "task-hf-correlations")
{
  ...
  Float_t leadingPtArr[] = {0.2, 0.5, 1.0, 2.0, 3.0};

  Int_t maxAssocPt = 4;
  Float_t assocPtArr[] = {0.2, 0.5, 1.0, 2.0, 3.0};
  ...
  Printf("Events with centrality: %d", (int)eventHist->Integral(eventHist->GetXaxis()->FindBin(6.), eventHist->GetXaxis()->FindBin(6.), //  6 = kCFStepReconstructed
                                                                eventHist->GetYaxis()->FindBin(0.), eventHist->GetYaxis()->FindBin(250.1))); // 250 = last bin of multiplicity (or 99.9 for centrality)
  ...
  //  plot example distributions for a wide range of Nch
  h->setCentralityRange(0, 49.9);
  h->setZVtxRange(-6.9, 6.9);
  hMixed->setCentralityRange(0, 49.9);
  hMixed->setZVtxRange(-6.9, 6.9);
  ...
  for (Int_t i = 0; i < maxLeadingPt; i++) {
    for (Int_t j = 0; j < maxAssocPt; j++) {
      ...
      // Below, the same event correlations is divided by mixed event after normalization 
      // for each pTtrig, pTassoc and mult bin (will be integrated over all Vz)

      //  set range of pTassoc which will remain fixed in the following calling functions
      ...
      for (Int_t mult = 0; mult < maxCentrality; mult++) {
        ...
        TH1* hist1 = 0;
        GetSumOfRatios(h, hMixed, &hist1, step, centralityArr[mult], centralityArr[mult + 1] - 0.01, leadingPtArr[i] + 0.01, leadingPtArr[i + 1] - 0.01, normalizePerTrigger);
    ...
    SetupRanges(h); // this is set to compare with Jasper
