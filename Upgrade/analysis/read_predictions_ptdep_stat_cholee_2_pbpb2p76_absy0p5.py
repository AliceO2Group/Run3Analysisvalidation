...
from ROOT import TH1F, TH2F, TCanvas, TFile, TGraph, TLatex, gPad, gStyle
...
def read_predictions(hadron="Omega_ccc"):
    ...
    if do2pipnorm == 1:
        cross_val_ptscaled = array(
            "f", [2 * 3.14 * a * b for a, b in zip(ptcand_val, cross_val)]
        )
    if do2pipnorm == 0:
    ...
    for i in range(nbins - 1):
        ...
        yvalue = grpred.Eval(i * width + width / 2.0)
        if i * width + width / 2.0 > 8.0:
            yvalue = 0.0
        histo.SetBinContent(i + 1, yvalue)
        print(i + 1, i * width + width / 2.0, yvalue)
    ...
    histo_norm = TH1F(
        "hdNdpt_norm",
        ";p_{T}; #Delta N/#Delta p_{T}, |y|<0.5, norm = 1",
        nbins,
        minb,
        maxb,
    )
    ...
    cpred.cd()
    gPad.SetLogy()
    hempty = TH2F("hempty", ";p_{T};dN/d p_{T}", 100, 0.0, 10.0, 100, miny, maxy)
